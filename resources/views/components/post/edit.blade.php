        @push('style')
        <!-- styles the Quill library -->
          <link href="https://cdn.jsdelivr.net/npm/quill@2.0.3/dist/quill.snow.css" rel="stylesheet" />
        @endpush

        <div class="max-w-4xl relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <!-- Modal header -->
              ...
              @method('PATCH')
                    ...
                    <div class="mb-3">
                      ...
                      <select name="category_id" id="category" class="@error('category_id') 
                        ...
                        <option value={{ $category->id }} @selected((old('category_id') ?? $post->category->id) == $category->id)>{{ $category->nama }}</option>
                        @endforeach
                      </select>
                       @error('category_id')
                         <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                      <label for="body" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Body</label><textarea id="body" name="body" rows="4" class="hidden @error('body') 
                         bg-red-50 border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 @enderror block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Write Post description here"  >{{ old('body') ?? $post->body }}
                        </textarea>
                        <div id="editor" style="color: !important white">{!! old('body', $post->body) !!}</div>
                          @error('body')
                         <p class="mt-2 text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                         @enderror
                    </div>
...
@push('script')
          ...
          <script>
            ...
            const quilEditor = document.querySelector('#editor');
            const postBody = document.querySelector('#body');

            postForm.addEventListener('submit', function(e) {
              const content = quilEditor.children[0].innerHTML;
              e.preventDefault();
